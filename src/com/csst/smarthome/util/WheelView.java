...
public class WheelView extends View {
	...
	private void initResourcesIfNecessary() 
	{
		if (itemsPaint == null) {
			...
			TEXT_SIZE=getContext().getResources().getDimensionPixelSize(R.dimen.ert);
	...
	private int getDesiredHeight(Layout layout) {
		...
		TEXT_SIZE=getContext().getResources().getDimensionPixelSize(R.dimen.ert);
		ADDITIONAL_ITEM_HEIGHT=getContext().getResources().getDimensionPixelSize(R.dimen.itemheigt);
		int desired = getItemHeight() * visibleItems - TEXT_SIZE * 2/5
	...
	protected void onDraw(Canvas canvas) {
		...
		if (itemsWidth > 0) {
			...
			TEXT_SIZE=getContext().getResources().getDimensionPixelSize(R.dimen.ert);
			canvas.translate(PADDING, -TEXT_SIZE/5);
			drawItems(canvas);
			drawValue(canvas);
			canvas.restore();
		}
